    ...
    <div class="container pt-5">
        <div class="row">
            <div class="col-md-12">
                    ...
                    <form id="form_formulario" method="POST">
                    @csrf
                    @foreach($nombre as $nombref)
                                <div class="form-row">
                                    <div class="form-group col-md-3">
                                        <label for="">Nombre Formulario</label>
                                        <input type="text" class="form-control" name="nombre_formulario" value ='{{$nombref->nombre_formulario}}'>
                                    </div>
                                </div>
                            @endforeach 
                    <div id="contenedor"> 
                             ...
                             <div class="row border rounded">
                                <input type="text" value="{{$dato1 -> id_enlace}}" name="id[]" hidden >
                                    <div class="col mt-2 mb-2">
                                        <select class="form-select data1" aria-label="Default select example" name="tablas[]" id="tablas" >
                                            <option value="{{$dato1->nombre_tabla}}" selected  hidden>{{$dato1->nombre_tabla}}</option>
                                    ...
                                    <div class="col mt-2 mb-2">
                                        <select class="form-select data2" aria-label="Default select example" name="campos[]" id="campos" >
                                            <option value="{{$dato1->nombre_campo}}" selected  hidden>{{$dato1->nombre_campo}}</option>
                                        </select>
                                    </div>
                                    <div class="col mt-2 mb-2">
                                    <select class="form-select puntero3" aria-label="Default select example" name="visibilidad[]" id="visibilidad">
                                    <?php
                                        if ($dato1->visibilidad_campo == 1) {
                                            echo "<option value='$dato1->visibilidad_campo'>Si</option>";
                                            echo "<option value='0'>No</option>";

                                        }else{
                                            echo "<option value='$dato1->visibilidad_campo'>No</option>";
